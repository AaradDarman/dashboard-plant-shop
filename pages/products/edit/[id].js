...
const Edit = ({ id }) => {
  ...
  useEffect(() => {
    if (router.pathname.includes("edit")) {
      if (inventory.length != sizes.length) {
        setInventory(
          sizes.map((size) => {
            let inventoryTarget = inventory.find(
  ...
  useEffect(() => {
    dispatch(getProduct(id))
      ...
      .then((originalPromiseResult) => {
        setName(originalPromiseResult.product.name);
        setDescription(originalPromiseResult.product.description);
        setSizes(
          originalPromiseResult.product.sizes.map((obj) => JSON.stringify(obj))
        );
        setCategory(originalPromiseResult.product.category);
        setCare(originalPromiseResult.product.care);
        setLight(originalPromiseResult.product.light);
        setWatering(originalPromiseResult.product.watering);
        setImages(originalPromiseResult.product.images);
        setInventory(originalPromiseResult.product.inventory);
      });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const AddProductSchema = Yup.object().shape({
    ...
    description: Yup.string()
      ...
      .max(900, "زیر عنوان وارد شده نباید بیشتر از 900 حرف باشد")
      .required("پر کردن این فیلد الزامی می باشد"),
    category: Yup.array()
